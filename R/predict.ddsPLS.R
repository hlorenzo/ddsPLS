...
#' Default to `TRUE`.
#' @param doDiagnosis boolean, wether or not to perform the diagnoses operations.
...
predict.ddsPLS <- function(object,X_test=NULL,toPlot=FALSE,doDiagnosis=T,
  ...
  getDiagnoses <- function(object,X_test,y_test_est){
    ...
    X_test_center <- X_test-matrix(rep(object$model$muX,n_test),nrow = n_test,byrow = T)
    X_train_center <- object$X-matrix(rep(object$model$muX,n_train),nrow = n_train,byrow = T)
    ## On the predictions
    ...
    # sigmaInv <- unlist(lapply(object$model$sdX,function(ss){
    #   if(ss>1e-9){out <- 1/ss}else{out <- 0};out}))
    ...
    list(# y=list(train=dist_y_Mahalanobis_train,test=dist_y_Mahalanobis_test),
      ...
      object=list(train=dist_Mahalanobis_train,test=dist_Mahalanobis_test))
  }
  diagnoses <- NULL
  if(is.null(X_test)){
    y_est <- object$Y_est
    ...
    n_test <- nrow(X_test)
    if(object$R==0){
      y_est <- matrix(rep(object$model$muY,n_test),nrow = n_test,byrow = T)
    }else{
      y_est <- (X_test-matrix(rep(object$model$muX,n_test),nrow = n_test,byrow = T))%*%object$model$B
      y_est <- y_est + matrix(rep(object$model$muY,n_test),nrow = n_test,byrow = T)
    }
    if(doDiagnosis & object$R>0){
      diagnoses <- getDiagnoses(object,X_test,y_est)
      if(toPlot){
        xlim <- range(c(diagnoses$t$train,diagnoses$t$test))
        ylim <- range(c(diagnoses$object$train,diagnoses$object$test))
        N <- 4
        par(mar=c(5, 4, 1, 0.5) + 0.1)
        layout(matrix(c(rep(2,N),4,rep(c(rep(1,N),3),N)),nrow = N+1,byrow = T))
        plot(diagnoses$t$train,diagnoses$object$train,col=1,pch=1,
             cex=cex,xlab="",ylab="",
             xlim=xlim,ylim=ylim)
        ...
        points(diagnoses$t$test,diagnoses$object$test,col="red",pch=16,cex=cex)
        ...
        dd <- density(diagnoses$object$train)
        ...
        title(main = expression("Density of"~d[bold(x)](hat(bold(x)),bold(x))),
              line = -2,cex.main=cex.text)
        dd <- density(diagnoses$object$test)
